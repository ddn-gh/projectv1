...
const EditTestPage = () => {
    ...
    fetch("https://asttestapp.onrender.com/auth/login", {
      ...
      .then((data) => {
        if (data.access_token) {
          ...
          handleEditImage(); // ลองส่งใหม่อีกครั้ง
        } else {
  ...
  return (
    <div
      className="editTestPage"
      style={{ maxWidth: "80%", width: "80%", padding: "20px" }}
    >
      <h1 style={{ textAlign: "center" }}>Create New Test</h1>
      ...
      >
        <Form.Group>
          <Form.Label>Test ID</Form.Label>
          <Form.Control type="number" value={testData.test_id} readOnly />
        </Form.Group>
        <Form.Group>
          <Form.Label>Bacteria Name</Form.Label>
          <Form.Control value={testData.bacteria_name} readOnly></Form.Control>
        </Form.Group>
        {errors["bacteria"] && (
          <p style={{ color: "red" }}>
